...
export class DatasourceTable extends Component<
...
> {
  ...
  handleSelect = () => {
    this.setState(
      prevState => ({
        ...
        selectedColumns: prevState.selected
          ? []
          : prevState.columns.map(column => column.columnName),
  ...
  loadTableData = async () => {
    ...
    const selectStarQuery = await getSelectStarQuery(
      databaseId,
      tableName,
      schemaName,
    );
  ...
  async getDatasourceTableColumnProps() {
    ...
    const columnData = (
      await fetchColumnData(databaseId, schemaName, tableName)
  ...
  componentDidUpdate(
    prevProps: Readonly<DatasourceTableProps>,
    prevState: Readonly<DatasourceTableProps>,
    snapshot?: any,
  ): void {
  ...
  handleGenerateDescription = async () => {
    ...
    const { description, columns } = await getTableDescription(
      databaseId,
      schemaName,
      tableName,
    );
      ...
      <div>
        ...
        >
          ...
          >
            ...
            <TextArea
              placeholder={
                description || 'What data does this database schema contain?'
              }
              value={description}
              onFocus={() => {
            ...
            <Table
              ...
              rowSelection={{
                ...
                onChange: (selectedRowKeys, selectedRows) => {
                  const selectedColumnNames = selectedRows.map(
                    row => row.columnName,
                  );
