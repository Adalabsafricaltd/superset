...
export class Datasource extends React.Component<DatasourceProps> {
  ...
  async componentDidMount() {
    ...
    if (schema.length === 0) {
      ...
      const schemaData: DatasourceSchemaProps[] = schema.map(
        (schema: DatabaseScemaData) => {
          return {
  ...
  async componentDidUpdate(
    prevProps: Readonly<DatasourceProps>,
    prevState: Readonly<{}>,
    snapshot?: any,
  ) {
      ...
      >
        ...
        >
          ...
          >
            {schema.map(schema => (
              <DatasourceSchema
                key={'d_schema' + schema.schemaName}
                {...schema}
                actions={this.props.actions}
