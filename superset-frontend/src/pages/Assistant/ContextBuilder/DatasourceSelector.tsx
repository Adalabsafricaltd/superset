...
export class DatasourceSelector extends Component<
...
> {
  ...
  componentDidUpdate(
    prevProps: Readonly<DatasourceSelectorProps>,
    prevState: Readonly<DatasourceSelectorState>,
    snapshot?: any,
  ): void {
    ...
    this.setState(prevState => {
      ...
      const selectedDatasources = updated.filter(datasource =>
        datasource.schema.some(schema =>
          (schema.tables || []).some(
            table => (table.selectedColumns || []).length > 0,
          ),
        ),
      );
  ...
  render() {
    ...
    const summary: ContextSelectionProps = {
      ...
      numSchemas: datasources.reduce(
        (acc, datasource) => acc + datasource.schema.length,
        0,
      ),
        ...
        >
          ...
          >
            ...
            >
              ...
              <p>
                Choose the Databases you want the assistant to have access to.
              </p>
            </div>
            <div style={{}}>
              {/* Loading Indicator */}
