 ...
 */
export class DatasourceSchema extends Component<
  DatasourceSchemaProps,
...
> {
  ...
  async getDatasourceTableProps() {
    ...
    const tableData: DatasourceTableProps[] = (
      await fetchTableData(databaseId, schemaName)
    ).map((table: DatabaseSchemaTableData) => {
      return {
  ...
  componentDidUpdate(
    prevProps: Readonly<DatasourceSchemaProps>,
    prevState: Readonly<DatasourceSchemaProps>,
    snapshot?: any,
  ): void {
  ...
  handleOnChange = (data: DatasourceTableProps) => {
    //update the state
    this.setState(
      (prevState: DatasourceSchemaProps) => {
        const newState = {
          ...prevState,
  ...
  handleGenerateDescription = async () => {
    ...
    setTimeout(() => {
      this.setState(prevState => {
        return {
          description:
            'This schema contains data related to sales and customers.',
          isDescriptionLoading: false,
        };
      });
    }, 2000);
    ...
    return (
      <>
        ...
        >
                ...
                <div style={{}}>
                  ...
                  <div>
                    <Input
                      ...
                      placeholder={
                        description ||
                        'What data does this database schema contain?'
                      }
                      value={description}
                      onFocus={() => {
                        this.handleDescriptionFocus(true);
                      }}
                      onBlur={() => {
                        this.handleDescriptionFocus(false);
                      }}
                      style={{
                        width: '100%',
                        padding: '10px',
              ...
              <div>
                {(tablesToShow || 15) < (tables || []).length && (
                  <Button
                    ...
                    onClick={this.loadMoreTables}
                  >
                    Load More
                  </Button>
                )}
                <Button onClick={this.openRefreshModal}>
          ...
          >
            <p>
              Are you sure you want to clear the selection and refresh the
              tables?
            </p>
          </Modal>
        </div>
