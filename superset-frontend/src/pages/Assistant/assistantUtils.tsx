...
export const getTableDescription = async (
  ...
  try {
    ...
    console.error(
      'getTableDescription: Error fetching table description:',
      error,
    );
    return {};
  }
...
export function cleanDatasourceProps(
  data: DatasourceProps[],
): DatasourceProps | undefined {
  if (!data) {
    return undefined;
  }
  const clean = data
    .map(datasource => filterSelectedSchemas(datasource))
    .filter(datasource => datasource.schema.length > 0);
  if (clean.length === 0) {
    return undefined;
  }
  return clean[0];
...
function filterSelectedSchemas(data: DatasourceProps): DatasourceProps {
  return {
    ...data,
    schema: data.schema
      .map(schema => filterSelectedTables(schema))
...
function filterSelectedTables(
  ...
  return {
    ...data,
    tables: data.tables
      .map(table => filterSelectedColumns(table))
...
export const getVizSuggestions = async (
  data: DatasourceProps[],
  purpose: string,
) => {
  ...
  try {
    ...
    const suggestions = responseJson.map((r: any) => {
      const suggestion: AssistantSuggestionProps = {
        ...
        backgroundColor:
          suggestionColors[Math.floor(Math.random() * suggestionColors.length)],
      };
      return suggestion;
    });
    ...
    console.error(
      'getVizSuggestions: Error fetching table description:',
      error,
    );
    return [];
  }
...
export function saveChartExample(
  viz_type: string,
  controls: any,
  formData: any,
) {
...
export function getChartControlValues(
  instruction: string,
  controls: {},
  formData: {},
) {
...
export function getChartExplanation(
  ...
  const endpoint = 'assistant/get-viz-explanation';
  const data = {
