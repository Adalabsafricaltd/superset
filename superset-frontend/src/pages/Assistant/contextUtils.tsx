...
export const fetchSchemaData = async (databaseId: number) => {
  ...
  try {
    ...
    const schemas: DatabaseScemaData[] = response.json.result.map(
      (schema: any) => {
...
export const fetchTableData = async (
  ...
  try {
    const response = await SupersetClient.get({ endpoint: endpoint });
    const tables: DatabaseSchemaTableData[] = response.json.result.map(
      (table: any) => {
 ...
 */
export const getSelectStarQuery = async (
  databaseId: number,
  tableName: string,
  schemaName: string,
) => {
...
export function readableColor(
  hex: string,
  thresholdBright: number = 150,
  thresholdDark: number = 140,
): string {
...
export async function getDomAsImageByteArray(selector: string) {
  ...
  if (!element) {
    console.error(
      `getDomAsImageByteArray Element with selector "${selector}" not found.`,
    );
    return;
  }
  ...
  try {
    ...
    console.error(
      'getDomAsImageByteArray Error converting DOM to image:',
      error,
    );
  }
  return retVal;
