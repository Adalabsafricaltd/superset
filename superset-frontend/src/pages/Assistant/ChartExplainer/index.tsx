...
import { getChartExplanation } from '../assistantUtils';
...
export function ChartExplainer(props: any) {
  const chart_selector = `.dashboard-chart-id-${props.id || '0'}`;
  const internalProps = {
    controls:
      props.controls ||
      getControlsState(props, props.formData || props.form_data),
    form_data: props.formData || null,
  ...
  useEffect(() => {
    ...
    getChartExplanation(
      internalProps.datasource,
      internalProps.controls,
      internalProps.form_data,
      chartImage,
    )
      .then(explanation => {
        setDescription({
